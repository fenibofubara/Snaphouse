...
//echo "<title>Grab a web page's title</title>"
	...
	public function count_sql($wheretocount){
		...
		echo"
			<h4 class='comments-heading' style = 'color:rgb(134,0,0)'> $comntno comments:</h4>	
		";
...
if($stmt = $this->con->prepare("INSERT INTO comment(name,email,comment,post_id,commenttime) VALUES(?,?,?,?,?) ")){
	...
	return "Repost Your Comment Please".$this->con->error;
	 ...
	 function latestpost($catyid){
    $stmt = $this->con->prepare("SELECT * FROM comment WHERE post_id= '$catyid' ORDER BY comment_id DESC");
		$stmt->execute();
		$arr = array();
		$stmt_result = $stmt->get_result(); // puts it in object form
		if(mysqli_num_rows($stmt_result)==0){
			echo"<span>No recent comment</sapn>";
		}
		else{
			while($row = $stmt_result->fetch_assoc()){
			$obj = new stdClass();
			$obj->name = $row["name"];
			$obj->time = $row["commenttime"];
			$obj->comment = $row["comment"];
			$arr[] = $obj;
		}
		}
